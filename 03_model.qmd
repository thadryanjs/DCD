...
from src.utils import get_preprocessor
```
...
# Data Audit & Type Verification
Confirm data types and structure before feature identification.
...
# Parity Check vs R Pipeline
selected_feats_path = Path("output/selected-features.csv")
if selected_feats_path.exists():
    selected_feats = pl.read_csv(selected_feats_path)["feature"].to_list()
    ml_feats = numeric_cols + categorical_cols
    
    missing_in_ml = set(selected_feats) - set(ml_feats)
    extra_in_ml = set(ml_feats) - set(selected_feats)
    
    print("\n--- Parity Check (vs selected-features.csv) ---")
    if not missing_in_ml and not extra_in_ml:
        print("SUCCESS: ML feature set matches R selected features exactly.")
    else:
        if missing_in_ml:
            print(f"MISSING in ML: {missing_in_ml}")
        if extra_in_ml:
            print(f"EXTRA in ML: {extra_in_ml}")
else:
    print("\n--- Parity Check ---")
    print("Skipped: output/selected-features.csv not found.")

print("\n--- Final Receipts ---")
print(f"Total Features Entering Pipeline: {len(numeric_cols) + len(categorical_cols)}")
...
def run_ml_pipeline(df, numeric_cols, categorical_cols, prefix="full"):
    ...
    # Write training split to artifact for consistency in 04
    train_patients_df = pd.DataFrame({"alias_filled": groups[train_idx]})
    train_patients_df.to_csv(data_dir / f"train_patients_{prefix}.csv", index=False)

    groups_train, groups_test = groups[train_idx], groups[test_idx]
    x_train, x_test = x_df.iloc[train_idx], x_df.iloc[test_idx]
    y_train, y_test = y[train_idx], y[test_idx]

    # Leakage Audit: Explicitly verify patient isolation
    leakage_set = set(groups_train).intersection(set(groups_test))
    leakage_count = len(leakage_set)
    print(f"Leakage Audit: {leakage_count} patients shared between train and test. {'✓ CLEAN' if leakage_count == 0 else '✗ LEAKAGE DETECTED'}")
    assert leakage_count == 0, f"Patient leakage detected: {leakage_set}"
    ...
    for name in pipelines.keys():
        ...
        for repeat in range(3):
            ...
            for fold_idx, (outer_train_idx, outer_test_idx) in enumerate(kf_outer.split(x_train, y_train, groups=groups_train)):
                ...
                # Renamed from gr_tr_out to avoid shadowing
                groups_outer_train = groups_train[outer_train_idx]
                groups_outer_test = groups_train[outer_test_idx]
                # Leakage Audit: Explicitly verify patient isolation
                leakage_set = set(groups_outer_train).intersection(set(groups_outer_test))
                leakage_count = len(leakage_set)
                if leakage_count > 0:
                    print(f"  ✗ LEAKAGE in Fold {fold_idx+1} (repeat {repeat}): {leakage_set}")
                assert leakage_count == 0, f"Patient leakage in CV fold {fold_idx} (repeat {repeat})"

                if name == "Logistic Regression":
                    stats = get_class_stats(y_te_out, groups_outer_test)
                    fold_composition.append({
                        "repeat": repeat + 1,
                        "fold": fold_idx + 1,
                        "n_patients": len(np.unique(groups_outer_test)),
                        **stats
                    })
                ...
                grid.fit(x_tr_out, y_tr_out, groups=groups_outer_train)
    ...
    for name in pipelines.keys():
        ...
        test_metrics[name] = auc_test

    with open(data_dir / f"model_params_{prefix}.json", "w") as f:
        json.dump(model_final_params, f)

    # Create Final Summary Table: Model, Test AUC, CV Low, CV Median, CV High
    summary_table = []
    for name in pipelines.keys():
        scores = cv_results_df.filter(pl.col("model") == name).filter(pl.col("metric") == "auc")["value"].to_numpy()
        clean_scores = scores[~np.isnan(scores)]
        ...
        summary_table.append({
            "model": name,
            "test_set_auc": test_metrics[name],
            "cv_low": np.min(clean_scores),
            "cv_median": np.median(clean_scores),
            "cv_high": np.max(clean_scores)
        })
    
    summary_df = pd.DataFrame(summary_table)
    summary_df.to_csv(plots_dir / f"final_performance_summary_{prefix}.csv", index=False)
    print(f"\n--- Final Performance Summary ({prefix}) ---")
