...
**Note on Interpretation:** The `estimate` in the output CSV is the log-odds per standard deviation of the patient mean (unadjusted marginal effect). These are univariate screens, not mutually adjusted effects. In `04_analyze-model.py`, these are exponentiated to produce Odds Ratios (OR).
...
dropped_at_patient <- setdiff(selected_feats, r_feats)

if (length(dropped_at_patient) > 0) {
  print("Note: Some selected features were dropped because they became constant at patient-level:")
  print(dropped_at_patient)
}
```
...
ggplot(plot_df, aes(or, feature)) +
  ...
  labs(
    ...
    subtitle = sprintf(
      "%d features; %d unstable fits excluded (MICE m=5, Rubin's Rules)",
