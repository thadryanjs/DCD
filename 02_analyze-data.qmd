...
The **First-Look logistic regression (GLM)** is the proper inferential model for this data structure. We analyze only the baseline (first) observation for each patient. This approach ensures that features precede the outcome process and aligns with the primary ML unit of analysis in the predictive model.
...
results are pooled using Rubin's rules to account for the uncertainty introduced by the imputation process.

Results are pooled across imputations for consistency. Features are scaled, so estimates are per standard deviation.
...
## First-Look Frames
Filter to the first observation per patient, drop the id, keep only columns with at least two
...
```{r}
# We defer selecting the first-look frame until we have the selected feature set from 01_explore-data.qmd
```
...
# Construct First-Look Frame
df_first_look <- df_filtered %>% 
  filter(observation == 1) %>% 
  select(-all_of(id), -any_of("observation"))

# Drop constant features
cols_before <- names(df_first_look)
df_first_look <- df_first_look[sapply(df_first_look, function(x) n_distinct(x, na.rm = TRUE) >= 2)]
cols_after <- names(df_first_look)
dropped <- setdiff(cols_before, cols_after)

if (length(dropped) > 0) {
  print(sprintf("Dropped %d constant features: %s", length(dropped), paste(dropped, collapse = ", ")))
}
...
analyze <- function(d) {
  ...
  for (f in setdiff(names(d), label)) {
    ...
    for (i in 1:m) {
      fit <- tryCatch({
        glm(as.formula(sprintf("%s ~ `%s`", label, f)),
          complete(imp, i),
          family = binomial
        )
      ...
      # Singularity check: NA in standard errors of coefficients
      if(any(is.na(summary(fit)$coefficients[, "Std. Error"]))) {
        singular <- singular + 1
      }
    }
    
    # Only pool if we have fits
    if(length(fits) == 0) {
      out <- rbind(out, data.frame(
        feature = f, estimate = NA, std_error = NA, p_value = NA,
        ci_low = NA, ci_high = NA,
        unstable = TRUE, separated_flag = separated,
        frac_singular = singular/m, frac_nonconv = nonconv/m
      ))
      next
    }
    ...
    out <- rbind(out, data.frame(
      ...
      frac_singular = singular/m,
      frac_nonconv = nonconv/m
    ))
  }
  out %>%
...
ggplot(plot_df, aes(or, feature)) +
  ...
  labs(
    ...
    subtitle = sprintf(
      "%d features; %d unstable fits excluded (First-Look, MICE m=5, Rubin's Rules)",
