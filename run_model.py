...
def run_ml_pipeline(df, numeric_cols, categorical_cols, prefix="full"):
    ...
    assert len(set(groups_train).intersection(set(groups_test))) == 0, "Patient leakage in train/test split"

    # Split Receipt
    train_patients = len(np.unique(groups_train))
    test_patients = len(np.unique(groups_test))

    def get_class_stats(y_vals, gr_vals):
        pos_mask = (y_vals == 1)
        neg_mask = (y_vals == 0)
        return {
            "pos_rows": np.sum(pos_mask),
            "neg_rows": np.sum(neg_mask),
            "pos_pts": len(np.unique(gr_vals[pos_mask])),
            "neg_pts": len(np.unique(gr_vals[neg_mask])),
        }

    train_stats = get_class_stats(y_train, groups_train)
    test_stats = get_class_stats(y_test, groups_test)

    print(
        f"Split ({prefix}):\n"
        f"  Train shape: {x_train.shape} ({train_patients} patients)\n"
        f"    Class 1: {train_stats['pos_rows']} rows, {train_stats['pos_pts']} patients\n"
        f"    Class 0: {train_stats['neg_rows']} rows, {train_stats['neg_pts']} patients\n"
        f"  Test shape: {x_test.shape} ({test_patients} patients)\n"
        f"    Class 1: {test_stats['pos_rows']} rows, {test_stats['pos_pts']} patients\n"
        f"    Class 0: {test_stats['neg_rows']} rows, {test_stats['neg_pts']} patients\n"
    ...
    for name in pipelines.keys():
        ...
        for repeat in range(3):
            ...
            for fold_idx, (outer_train_idx, outer_test_idx) in enumerate(kf_outer.split(x_train, y_train, groups=groups_train)):
                ...
                gr_te_out = groups_train[outer_test_idx]
                assert len(set(gr_tr_out).intersection(set(gr_te_out))) == 0, f"Patient leakage in CV fold {fold_idx} (repeat {repeat})"

                if name == "Logistic Regression":
                    stats = get_class_stats(y_te_out, gr_te_out)
                    fold_composition.append({
                        "repeat": repeat + 1,
                        "fold": fold_idx + 1,
                        "n_patients": len(np.unique(gr_te_out)),
                        **stats
                    })
            ...
            for m, scores in repeat_metrics.items():
                median_s = np.nanmedian(scores)
                min_s = np.nanmin(scores)
                max_s = np.nanmax(scores)
                print(f"    {m:12s}: {median_s:.3f} [{min_s:.3f}, {max_s:.3f}] [n={len(scores)}]")
    ...
    print("\n--- CV Fold Composition ---")
    comp_df = pd.DataFrame(fold_composition)
